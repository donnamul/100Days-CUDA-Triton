#include <cuda_runtime.h>
#include <vector>
#include <iostream>
#include <algorithm>
#include <cmath>
#include <cstdlib>

#define NUM_DATA 1024

__device__ float simple_rand(unsigned int seed, unsigned int idx) {
    unsigned int state = seed ^ (idx * 0x9e3779b9);
    state ^= state << 13;
    state ^= state >> 17;
    state ^= state << 5;
    return static_cast<float>(state) / static_cast<float>(0xFFFFFFFF);
}

__global__ void seeded_dropout_kernel(float *vec, float p, unsigned int seed, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        float x = vec[idx];
        float random = simple_rand(seed, idx);
        vec[idx] = (random > p) ? x / (1.0f - p) : 0.0f;
    }
}

int main() {
    int n = NUM_DATA;
    float p = 0.5f;
    unsigned int seed = 123;
    
    std::vector<float> h_vec(n);
    for (int i = 0; i < n; i++) {
        h_vec[i] = 20.0f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX) - 10.0f;
    }

    std::vector<float> h_ref(n);
    for (int i = 0; i < n; i++) {
        unsigned int state = seed ^ (i * 0x9e3779b9);
        state ^= state << 13;
        state ^= state >> 17;
        state ^= state << 5;
        float random = static_cast<float>(state) / static_cast<float>(0xFFFFFFFF);
        h_ref[i] = (random > p) ? h_vec[i] / (1.0f - p) : 0.0f;
    }

    float *d_vec;
    cudaMalloc(&d_vec, n * sizeof(float));
    cudaMemcpy(d_vec, h_vec.data(), n * sizeof(float), cudaMemcpyHostToDevice);
    int threadsPerBlock = 256;
    int blocksPerGrid = (n + threadsPerBlock - 1) / threadsPerBlock;
    seeded_dropout_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_vec, p, seed, n);

    cudaMemcpy(h_vec.data(), d_vec, n * sizeof(float), cudaMemcpyDeviceToHost);

    cudaFree(d_vec);

    float max_error = 0.0f;
    float max_abs_value = 0.0f;
    for (int i = 0; i < n; i++) {
        max_error = std::max(max_error, std::abs(h_vec[i] - h_ref[i]));
        max_abs_value = std::max(max_abs_value, std::max(std::abs(h_vec[i]), std::abs(h_ref[i])));
    }
    std::cout << "Max error: " << max_error << std::endl;

    float max_error_percentage = 0.0f;
    if (max_abs_value > 1e-12f) {
        max_error_percentage = (max_error / max_abs_value) * 100.0f;
    }
    std::cout << "Max error percentage: " << max_error_percentage << "%" << std::endl;

    return 0;
}
